...
def connect_to_stock():
    ...
    def operations(title, text, command):
        global idEntry, amtEntry, dateEntry, secEntry, window
        window = Toplevel()
        ...
        if title == 'Update Operation':
            ...
            amtEntry.insert(0, content['values'][1])
            dateEntry.insert(0, content['values'][2])
            secEntry.insert(0, content['values'][3])

    def update_data():
        indexing = stockTable.focus()
        content = stockTable.item(indexing)
        stock_id = content['values'][0]
        for stock in shoe_stock:
            if stock['id'] == stock_id:
                stock['amt'] = amtEntry.get()
                stock['date'] = dateEntry.get()
                stock['section'] = secEntry.get()
                break
        messagebox.showinfo('Success', f'Stock Id - {stock_id} is modified Successfully', parent=window)
        window.destroy()
        view()
    ...
    def delete():
        def delete_data():
            ...
            stock_id = content['values'][0]
            global shoe_stock
            shoe_stock = [stock for stock in shoe_stock if stock['id'] != stock_id]
            messagebox.showinfo('Deleted', f'{stock_id} Stock ID row is Deleted Successfully', parent=window)
            window.destroy()
            view()
    ...
    def search_data():
        ...
        for stock in shoe_stock:
            if (idEntry.get() and stock['id'] == idEntry.get()) or \
               (amtEntry.get() and stock['amt'] == amtEntry.get()) or \
               (dateEntry.get() and stock['date'] == dateEntry.get()) or \
               (secEntry.get() and stock['section'] == secEntry.get()):
                stockTable.insert('', END, values=(stock['id'], stock['amt'], stock['date'], stock['section']))
        window.destroy()
    ...
    def add_data():
        if idEntry.get()=='' or amtEntry.get()=='' or dateEntry.get()=='' or secEntry.get()=='':
            ...
            for stock in shoe_stock:
                if stock['id'] == idEntry.get():
                    messagebox.showerror('Error', 'Stock ID cannot be repeated', parent=window)
                    return
            shoe_stock.append({
                'id': idEntry.get(),
                'amt': amtEntry.get(),
    ...
    stockTable = ttk.Treeview(middleFrame, columns=('id','amt','date','section'))
    stockTable.pack(fill=BOTH, expand=1)

    stockTable.heading('id', text = 'Stock ID')
    stockTable.column('id', anchor=CENTER)
    stockTable.heading('amt', text = 'Amount')
    stockTable.column('amt', anchor=CENTER)
    stockTable.heading('date', text = 'Stock Date')
    stockTable.column('date', anchor=CENTER)
    ...
    slide_text(heading_label,'Shoe Stock Table')
    root.mainloop()
...
def connect_to_customer():
    ...
    def update_data():
        indexing = customerTable.focus()
        content = customerTable.item(indexing)
        cust_id = content['values'][0]
        for cust in customers:
            if cust['id'] == cust_id:
                cust['name'] = nameEntry.get()
                cust['age'] = ageEntry.get()
                cust['phno'] = phnoEntry.get()
                cust['addr'] = addrEntry.get()
                break
        messagebox.showinfo('Success',f'Customer Id - {cust_id} is modified Successfully',parent=window)
        window.destroy()
        view()
    ...
    def delete():
        def delete_data():
            ...
            global customers
            customers = [cust for cust in customers if cust['id'] != content_id]
            messagebox.showinfo('Deleted',f'{content_id} customer ID row is Deleted Successfully',parent=window)
            window.destroy()
            view()
    ...
    def search_data():
        ...
        for cust in customers:
            if (idEntry.get() and cust['id'] == idEntry.get()) or \
               (nameEntry.get() and cust['name'] == nameEntry.get()) or \
               (ageEntry.get() and cust['age'] == ageEntry.get()) or \
               (phnoEntry.get() and cust['phno'] == phnoEntry.get()) or \
               (addrEntry.get() and cust['addr'] == addrEntry.get()):
                customerTable.insert('',END,values=(cust['id'], cust['name'], cust['age'], cust['phno'], cust['addr']))
        window.destroy()
    ...
    def add_data():
        if idEntry.get()=='' or nameEntry.get()=='' or phnoEntry.get()=='' or ageEntry.get()=='' or addrEntry.get()=='':
            ...
            for cust in customers:
                if cust['id'] == idEntry.get():
                    messagebox.showerror('Error','Customer ID cannot be repeated',parent=window)
                    return
            customers.append({
                'id': idEntry.get(),
                'name': nameEntry.get(),
